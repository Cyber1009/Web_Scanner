...
class MySpider(scrapy.Spider):
    ...
    def __init__(self, urls=None, keywords=None, result_queue=None, invalid_urls=None, *args, **kwargs):
        super(MySpider, self).__init__(*args, **kwargs)
        self.urls = urls if urls else []
        self.keywords = keywords if keywords else []
        self.result_queue = result_queue  # Store the queue for sending results back
        self.invalid_urls = invalid_urls if invalid_urls else []  # Store invalid URLs
    ...
    def handle_error(self, failure):
        """Handle failed requests (non-accessible URLs)."""
        url = failure.request.url
        self.invalid_urls.append(url)  # Log failed URLs as invalid
