...
from test_spider import MySpider  # Import spider class
...
def run_scrapy_spider(urls, keywords, result_queue, invalid_urls):
    ...
    # Pass the URLs, keywords, result queue, and invalid URLs list to the spider
    process.crawl(MySpider, urls=urls, keywords=keywords, result_queue=result_queue, invalid_urls=invalid_urls)
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("Start Scan"):
                ...
                if not url_column_error and keywords:
                    # Collect valid URLs only after the scan starts
                    ...
                    process = multiprocessing.Process(target=run_scrapy_spider,
                                                      args=(urls, keywords, result_queue, invalid_urls))
                    process.start()
                    ...
                    # Check if the result is a DataFrame before sorting
