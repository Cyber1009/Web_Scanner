...
def parse_html_and_find_keywords(content, keywords, flexible_search):
    ...
    # Extract text from the <body> tag, ignoring irrelevant sections
    body = soup.body
    if body:
...
def calculate_relevance_score(texts, keyword_counts, global_tfidf, ngram_length=2):
    relevance_scores = []
    ...
    for i, text in enumerate(texts):
        ...
        # Normalize match count using log length to reduce penalty for long texts
        normalized_match_score = match_count / (1 + content_length ** 0.5)

        # Scale the TF-IDF score to match normalized match score range
        tfidf_score = sum(global_tfidf.get(keyword, 0) * keyword_counts[i].get(keyword, 0) for keyword in keyword_counts[i])
        scaled_tfidf_score = tfidf_score / (1 + sum(global_tfidf.values()))

        # Combine both scores with appropriate weighting
        weighted_score = 0.7 * normalized_match_score + 0.3 * scaled_tfidf_score

        relevance_scores.append(max(weighted_score, 0.0))
